...
if ("document" in self && ("classList" in document.createElement("_") ? ! function() {
        ...
        if (repo_id) {
            if (!localStorage["stargazers"] || Date.now() - parseFloat(localStorage["stargazers_timestamp"]) > 9e4)
                pegasus("https://api.github.com/repos/" + repo_id).then(function(t, e) {
                    var n = t.stargazers_count;
                    n > 1e4 ? n = (n / 1e3).toFixed(0) + "k" : n > 1e3 && (n = (n / 1e3).toFixed(1) + "k");
                    localStorage["stargazers"] = n;
                    localStorage["stargazers_timestamp"] = Date.now();
                    document.querySelector(".repo-stars .count").textContent = n;
                }, function(t, e) {
                    //console.error(t, e.status)
                });

            else
               document.querySelector(".repo-stars .count").textContent =  localStorage["stargazers"];
        }
