...
var Flatpickr = function () {
	function Flatpickr(element, config) {
		...
		var self = this;
		if (element._flatpickr) self.destroy(element._flatpickr);
		...
		if (!self.isMobile) self._fp.build();
		...
		if (self.selectedDates.length) {
			if (self.config.enableTime) self._fp.setHoursFromDate();
	...
	_createClass(Flatpickr, [{
		...
		key: "changeMonth",
		value: function changeMonth(value, is_offset) {
			this.currentMonth = typeof is_offset === "undefined" || is_offset ? this.currentMonth + value : value;

			this._fp.handleYearChange();
			this._fp.updateNavigationCurrentMonth();
			this._fp.buildDays();

			if (!this.config.noCalendar) this.days.focus();
		...
		value: function isDateInRange(date) {
			...
			return this.compareDates(date, this.selectedDates[0]) >= 0 && this.compareDates(date, this.selectedDates[1]) <= 0;
		}
	}, {
		key: "isDateSelected",
		value: function isDateSelected(date) {
			for (var i = 0; i < this.selectedDates.length; i++) {
				if (this.compareDates(this.selectedDates[i], date) === 0) return "" + i;
			}
		...
		value: function isEnabled(dateToCheck) {
			if (this.config.minDate && this.compareDates(dateToCheck, this.config.minDate) < 0 || this.config.maxDate && this.compareDates(dateToCheck, this.config.maxDate) > 0) return false;

			if (!this.config.enable.length && !this.config.disable.length) return true;
			...
			for (var i = 0, d; i < array.length; i++) {
				...
				if (d instanceof Function && d(dateToCheck)) // disabled by function
					return bool;else if ((d instanceof Date || typeof d === "string") && this.parseDate(d, true).getTime() === dateToCheck.getTime())
					// disabled by date string
					return bool;else if ( // disabled by range
				(typeof d === "undefined" ? "undefined" : _typeof(d)) === "object" && d.from && d.to && dateToCheck >= this.parseDate(d.from) && dateToCheck <= this.parseDate(d.to)) return bool;
			}
		...
		key: "jumpToDate",
		value: function jumpToDate(jumpDate) {
			jumpDate = jumpDate ? this.parseDate(jumpDate) : this.latestSelectedDateObj || (this.config.minDate > this.now ? this.config.minDate : this.now);
		...
		value: function open(e) {
			...
			if (!self.config.static && !self.config.inline) self.positionCalendar();
		...
		value: function parseDate(date) {
			...
			if (date.toFixed || timestamp.test(date)) // timestamp
				...
				if (date === "today") {
					date = new Date();
					timeless = true;
				} else if (this.config.parseDate) date = this.config.parseDate(date);else if (timeRegex.test(date)) {
					// time picker
			...
			if (this.config.utc && !date.fp_isUTC) date = date.fp_toUTC();
		...
		value: function redraw() {
			if (this.config.noCalendar || this.isMobile) return;
		...
		value: function setDate(date, triggerChange) {
			...
			self.selectedDates = (Array.isArray(date) ? date.map(self.parseDate) : [self.parseDate(date)]).filter(function (d) {
				return d instanceof Date && self.isEnabled(d);
			});
			...
			if (triggerChange) self.triggerEvent("Change");
		}
	}, {
		key: "toggle",
		...
		get: function get() {
			...
			return {
				...
				setHoursFromInputs: function setHoursFromInputs() {
					...
					if (self.minDateHasTime && self.compareDates(self.latestSelectedDateObj, self.config.minDate) === 0) {
						hours = Math.max(hours, self.config.minDate.getHours());
						if (hours === self.config.minDate.getHours()) minutes = Math.max(minutes, self.config.minDate.getMinutes());
					} else if (self.maxDateHasTime && self.compareDates(self.latestSelectedDateObj, self.config.maxDate) === 0) {
				...
				setHoursFromDate: function setHoursFromDate(dateObj) {
					var date = dateObj || self.latestSelectedDateObj;

					if (date) self._fp.setHours(date.getHours(), date.getMinutes(), date.getSeconds());
				},
				setHours: function setHours(hours, minutes, seconds) {
					if (self.selectedDates.length) {
				...
				bind: function bind() {
					if (self.config.wrap) {
						["open", "close", "toggle", "clear"].forEach(function (el) {
							try {
								self.element.querySelector("[data-" + el + "]").addEventListener("click", self[el].bind(self));
							} catch (e) {
								//
							}
						});
					...
					if (self.isMobile) return self._fp.setupMobile();

					self.debouncedResize = self._fp.debounce(self._fp.onResize, 50);
					self.triggerChange = function () {
						return self.triggerEvent("Change");
					};
					self.debouncedChange = self._fp.debounce(self.triggerChange, 1000);

					if (self.config.mode === "range") self.days.addEventListener("mouseover", self._fp.onMouseOver);
					...
					if (self.config.clickOpens) (self.altInput || self.input).addEventListener("focus", self.open);
					...
					if (!self.config.noCalendar) {
						...
						self.currentYearElement.addEventListener("wheel", function (e) {
							return self._fp.debounce(self._fp.yearScroll(e), 50);
						});
					...
					if (self.config.enableTime) {
						self.timeContainer.addEventListener("wheel", function (e) {
							return self._fp.debounce(self._fp.updateTime(e), 5);
						});
				...
				incrementNumInput: function incrementNumInput(e, delta) {
				...
				createNumberInput: function createNumberInput(inputClassName) {
					var wrapper = self._fp.createElement("div", "numInputWrapper"),
					...
					arrowUp.addEventListener("click", function (e) {
						return self._fp.incrementNumInput(e, 1);
					});
				...
				build: function build() {
					...
					if (!self.config.noCalendar) {
						...
						if (self.config.weekNumbers) self.innerContainer.appendChild(self._fp.buildWeeks());
					...
					if (self.config.enableTime) fragment.appendChild(self._fp.buildTime());
				...
				createDay: function createDay(className, date, dayNumber) {
					var dateIsEnabled = self.isEnabled(date),
					    dayElement = self._fp.createElement("span", "flatpickr-day " + className, date.getDate());

					dayElement.dateObj = date;

					if (self.compareDates(date, self.now) === 0) dayElement.classList.add("today");
					...
					if (dateIsEnabled) {
						...
						if (self.isDateSelected(date)) {
							...
							if (self.config.mode === "range") {
								dayElement.classList.add(self.compareDates(date, self.selectedDates[0]) === 0 ? "startRange" : "endRange");
					...
					if (self.config.mode === "range") {
						if (self.isDateInRange(date) && !self.isDateSelected(date)) dayElement.classList.add("inRange");
				...
				buildDays: function buildDays() {
					...
					var daysInMonth = self._fp.utils.getDaysinMonth(),
					...
					for (var i = 0; dayNumber <= self.prevMonthDays; i++, dayNumber++) {
						days.appendChild(self._fp.createDay("prevMonthDay", new Date(self.currentYear, self.currentMonth - 1, dayNumber), dayNumber));
					}

					// Start at 1 since there is no 0th day
					for (dayNumber = 1; dayNumber <= daysInMonth; dayNumber++) {
						days.appendChild(self._fp.createDay("", new Date(self.currentYear, self.currentMonth, dayNumber), dayNumber));
					}

					// append days from the next month
					for (var dayNum = daysInMonth + 1; dayNum <= 42 - self.firstOfMonth; dayNum++) {
						days.appendChild(self._fp.createDay("nextMonthDay", new Date(self.currentYear, self.currentMonth + 1, dayNum % daysInMonth), dayNum));
					}

					self.days.appendChild(days);
					return self.days;
				},
				buildMonthNav: function buildMonthNav() {
					...
					var yearInput = self._fp.createNumberInput("cur-year");
				...
				buildTime: function buildTime() {
					...
					var separator = self._fp.createElement("span", "flatpickr-time-separator", ":");

					var hourInput = self._fp.createNumberInput("flatpickr-hour");
					self.hourElement = hourInput.childNodes[0];

					var minuteInput = self._fp.createNumberInput("flatpickr-minute");
					...
					if (self.config.enableSeconds) {
						...
						var secondInput = self._fp.createNumberInput("flatpickr-second");
						...
						self.timeContainer.appendChild(self._fp.createElement("span", "flatpickr-time-separator", ":"));
						self.timeContainer.appendChild(secondInput);
					}

					if (!self.config.time_24hr) {
						// add self.amPM if appropriate
						self.amPM = self._fp.createElement("span", "flatpickr-am-pm", ["AM", "PM"][self.hourElement.value > 11 | 0]);
				...
				buildWeekdays: function buildWeekdays() {
					if (!self.weekdayContainer) self.weekdayContainer = self._fp.createElement("div", "flatpickr-weekdays");
				...
				buildWeeks: function buildWeeks() {
					...
					self.weekWrapper.appendChild(self._fp.createElement("span", "flatpickr-weekday", self.l10n.weekAbbreviation));
					self.weekNumbers = self._fp.createElement("div", "flatpickr-weeks");
					self.weekWrapper.appendChild(self.weekNumbers);

					return self.weekWrapper;
				},
				handleYearChange: function handleYearChange(newYear) {
				...
				onKeyDown: function onKeyDown(e) {
					if (self.isOpen) {
						switch (e.which) {
							case 13:
								if (self.timeContainer && self.timeContainer.contains(e.target)) self._fp.updateValue();else self._fp.selectDate(e);
							...
							case 37:
								if (e.target !== self.input & e.target !== self.altInput) self.changeMonth(-1);
								break;

							case 38:
								e.preventDefault();

								if (self.timeContainer && self.timeContainer.contains(e.target)) self._fp.updateTime(e);else {
									self.currentYear++;
									self.redraw();
								}
				...
				onMouseOver: function onMouseOver(e) {
					...
					for (var t = rangeStartDate; t < rangeEndDate; t += self._fp.utils.duration.DAY) {
					...
					for (var timestamp = self.days.childNodes[0].dateObj.getTime(), i = 0; i < 42; i++, timestamp += self._fp.utils.duration.DAY) {
				...
				onResize: function onResize() {
					if (self.isOpen && !self.config.static && !self.config.inline) self.positionCalendar();
				},
				...
				selectDate: function selectDate(e) {
					...
					if (self.config.mode === "single") {
						...
						var selectedIndex = self.isDateSelected(selectedDate);
					...
					if (self.minDateHasTime && self.config.enableTime && self.compareDates(selectedDate, self.config.minDate) === 0) self._fp.setHoursFromDate(self.config.minDate);
					...
					if (self.config.mode === "range" && self.selectedDates.length === 1) self._fp.onMouseOver(e);

					self.triggerEvent("Change");
				},
				setupDates: function setupDates() {
					self.selectedDates = [];
					self.now = new Date();
					...
					self.selectedDates = self.selectedDates.filter(function (d) {
						return d instanceof Date && d.getTime() && self.isEnabled(d);
					});
				...
				setupInputs: function setupInputs() {
					self.input = self.config.wrap ? self.element.querySelector("[data-input]") : self.element;

					self.input.classList.add("flatpickr-input");
					if (self.config.altInput) {
						// replicate self.element
						self.altInput = self._fp.createElement(self.input.nodeName, self.config.altInputClass);
				...
				setupMobile: function setupMobile() {
					...
					if (self.selectedDates.length) {
						self.mobileInput.defaultValue = self.mobileInput.value = self.formatDate(self.mobileFormatStr, self.selectedDates[0]);
					}

					if (self.config.minDate) self.mobileInput.min = self.formatDate("Y-m-d", self.config.minDate);

					if (self.config.maxDate) self.mobileInput.max = self.formatDate("Y-m-d", self.config.maxDate);
				...
				updateValue: function updateValue() {
					...
					if (self.isMobile) {
						self.mobileInput.value = self.selectedDates.length ? self.formatDate(self.mobileFormatStr, self.latestSelectedDateObj) : "";
					}

					var joinChar = self.config.mode !== "range" ? "; " : self.l10n.rangeSeparator;

					self.input.value = self.selectedDates.map(function (dObj) {
						return self.formatDate(self.config.dateFormat, dObj);
					}).join(joinChar);

					if (self.config.altInput) {
						self.altInput.value = self.selectedDates.map(function (dObj) {
							return self.formatDate(self.config.altFormat, dObj);
						}).join(joinChar);
					}

					self.triggerEvent("ValueUpdate");
				},
				yearScroll: function yearScroll(e) {
					e.preventDefault();
		...
		get: function get() {
			...
			return {
				...
				F: function F(date) {
					return self._fp.utils.monthToStr(self.formats.n(date) - 1, false);
				},
				...
				M: function M(date) {
					return self._fp.utils.monthToStr(date.getMonth(), true);
				},
		...
		key: "latestSelectedDateObj",
		get: function get() {
			if (this.selectedDates.length) return this.selectedDates[this.selectedDates.length - 1];
			return null;
		}
	}], [{
		key: "localize",
		value: function localize(l10n) {
			_extends(Flatpickr.l10ns.default, l10n || {});
		}
	}]);
