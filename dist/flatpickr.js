...
function Flatpickr(element, config) {
	...
	function setHours(hours, minutes, seconds) {
		...
		if (!self.config.time_24hr) self.amPM.textContent = latestSelectedDateObj().getHours() >= 12 ? "PM" : "AM";
	...
	function selectDate(e) {
		if (self.config.allowInput && (e.which === 13 || e.which === 9) && e.target === (self.altInput || self.input)) return self.setDate((self.altInput || self.input).value), e.target.blur();
