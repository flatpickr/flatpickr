...
function Flatpickr(element, config) {
	...
	function bind() {
		...
		self.triggerChange = function () { return triggerEvent("Change"); };
		...
		if (!self.config.noCalendar) {
			self.prevMonthNav.addEventListener("click", function () { return changeMonth(-1); });
			self.nextMonthNav.addEventListener("click", function () { return changeMonth(1); });
	...
	function buildDays() {
		...
		for (dayNumber = 1; dayNumber <= daysInMonth; dayNumber++) {
			...
			var dayElement$1 = createElement(
				"span",
				dateIsDisabled
			...
			if (!dateIsDisabled) {
				dayElement$1.tabIndex = 0;

				if (equalDates(currentDate, self.now))
					dayElement$1.classList.add("today");

				if (isDateSelected(currentDate)){
					dayElement$1.classList.add("selected");
					self.selectedDateElem = dayElement$1;

					if (self.config.mode === "range") {
						dayElement$1.className +=	equalDates(currentDate, self.selectedDates[0])
							? " startRange"
							: self.selectedDates.length > 1
								? " endRange"
								: "";
			...
			triggerEvent("DayCreate", dayElement$1);
			days.appendChild(dayElement$1);
		}

		// append days from the next month
		for (var dayNum = daysInMonth + 1; dayNum <= 42 - self.firstOfMonth; dayNum++) {
			var curDate$1 = new Date(
				self.currentYear,
				self.currentMonth + 1,
				dayNum % daysInMonth,
				0, 0, 0, 0, 0
			),
				dateIsEnabled$1 = isEnabled(curDate$1),
				dayElement$2 = createElement(
					"span",
					"flatpickr-day nextMonthDay"
						+ (!dateIsEnabled$1 ? " disabled" : "")
						+ (isDateInRange(curDate$1) ? " inRange" : "")
						+ (
							(
								self.selectedDates.length === 1 &&
								(curDate$1 < self.minRangeDate || curDate$1 > self.maxRangeDate)
							) ? " notAllowed" : ""
						)
						+ (isDateSelected(curDate$1) !== false ? " selected" : ""),
					dayNum % daysInMonth
				);
			...
			if (self.config.weekNumbers && dayNum % 7 === 1) {
				self.weekNumbers.insertAdjacentHTML(
					"beforeend",
					"<span class='disabled flatpickr-day'>" + self.getWeek(curDate$1) + "</span>"
				);
			}

			if (dateIsEnabled$1)
				dayElement$2.tabIndex = 0;

			else if (self.selectedDates[0] && curDate$1 > self.minRangeDate && curDate$1 < self.selectedDates[0])
				self.minRangeDate = curDate$1;

			else if (self.selectedDates[0] && curDate$1 < self.maxRangeDate && curDate$1 > self.selectedDates[0])
				self.maxRangeDate = curDate$1;


			triggerEvent("DayCreate", dayElement$2);
			days.appendChild(dayElement$2);
		}
	...
	function formatDate(frmt, dateObj) {
		...
		return chars.map(function (c, i) { return self.formats[c] && chars[i - 1] !== "\\"
	...
	function onMouseOver(e) {
		...
		for (
			var timestamp = getDateFromElement(self.days.childNodes[0]).getTime(), i = 0;
			i < 42;
	...
	function parseConfig() {
		...
		var userConfig = Object.assign({}, self.instanceConfig, self.element.dataset || {});
	...
	function parseDate(date, timeless) {
	...
	function setupDates() {
		...
		if (Array.isArray(inputDate))
			...
			self.selectedDates = self.selectedDates.filter(function (d) { return d instanceof Date; });
		}
	...
	function setupHelperFunctions() {
		self.utils = {
			...
			monthToStr: function (monthNumber, short) {
					if ( short === void 0 ) short = self.config.shorthandCurrentMonth;
	...
	function triggerEvent(event, data) {
		if (self.config["on" + event]) {
			var hooks = Array.isArray(self.config["on" + event])
	...
	function updateNavigationCurrentMonth() {
		...
		if (self.config.minDate) {
			self.currentYearElement.min = self.config.minDate.getFullYear();
			var hidePrevMonthArrow = self.currentYear === self.config.minDate.getFullYear()
		...
		if (self.config.maxDate) {
			var hideNextMonthArrow = self.currentYear === self.config.maxDate.getFullYear()
	...
	function updateValue(readTimeInput) {
		if ( readTimeInput === void 0 ) readTimeInput = true;
		...
		switch (self.config.mode) {
			...
			case "multiple":
				self.input.value = self.selectedDates.map(function (dObj) { return formatDate(self.config.dateFormat, dObj); }).join("; ");
				if (self.altInput)
					self.altInput.value = self.selectedDates.map(function (dObj) { return formatDate(self.config.altFormat, dObj); }).join("; ");
				break;

			case "range":
				if (self.selectedDates.length === 2)
					(self.altInput || self.input).value = self.selectedDates.map(function (dObj) { return formatDate(self.config.dateFormat, dObj); }).join(" to ");
...
Flatpickr.localize = function (l10n) {
	Object.keys(l10n).forEach(function (k) { return Flatpickr.l10n[k] = l10n[k]; });
};
